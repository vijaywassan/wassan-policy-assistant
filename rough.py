...
def build_index():
    ...
    qdrant_client = QdrantClient(url="http://15.206.197.214:6333/", timeout=60)
    vector_store = QdrantVectorStore(client=qdrant_client, collection_name="rag_docs")
...
if not st.session_state["authenticated"]:
    ...
    def rebuild_index_from_docs(uploaded_files):
        ...
        # ✅ Get DB count after rebuild
        count = qdrant_client.count(collection_name="rag_docs").count
    
        # Show success + count in sidebar
        st.sidebar.success("✅ Index rebuilt with new documents!")
        status=st.sidebar.info(f"📊 Current vector count in DB: {count}")
    
        # Optionally return count if you want to use it elsewhere
        return status
